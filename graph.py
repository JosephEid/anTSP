...
class Graph(object):
    ...
    Graph of nodes
    """
    def __init__(self, nodes, alpha, beta, decay, min_pheromone=.01,
    ...
    def sum_others(self, i, j, available):
        set_copy = available.copy()
        set_copy.remove(j)
        sum = 0
        for j in set_copy:
            sum += (self.get_pheromone(i, j) ** self.alpha) * (self.get_distance(i, j) ** -self.beta)

        return sum

    def get_probability(self, i, j, available):
        """
        Returns probability of going from ith node to the jth
        """
        return (
            (self.get_pheromone(i, j) ** self.alpha) * (self.get_distance(i, j) ** -self.beta)/self.sum_others(i, j, available)
        )
    ...
    def find_shortest_path(self, gens):
        """
        ...
        for i in range(gens):
            ...
            __main__.update_message("Generation: "+ str(i+1) +"\n" + " Shortest Path: " + str(shortest_path))
